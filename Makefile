...
srcdir=.
abs_srcdir=/home/chad/wasm-ghdl/ghdl/.
GNATMAKE=gnatmake
ADA_FLAGS=-O
MAKE=make
prefix=/tmp/ghdl-wasm
backend=mcode
libdirsuffix=lib
...
llvm_be=llvm6
build_mode=
ghdl_version=7.0.0-dev
libghdl_version=7_0_0_dev
enable_werror=false
enable_checks=true
...
copy-sources.gcc: version.ads scripts/gcc/Make-lang.in
	...
	$(CP) -p $(srcdir)/src/grt/grt-asserts.ad? $(gcc_vhdl_dir)
	...
	case $$base_ver in \
	 ...
	 15.*)     	    gcc_ortho_lang=ortho-lang-15.c ;; \
	 *) echo "Mismatch gcc version from $(gcc_src_dir)"; \
	    echo "Need gcc version 4.9.x, 5.x to 15.x"; \
	    exit 1 ;; \
	esac; \
...
#	For gcc >= 12.x, use .cc extension
	base_ver=`cat $(gcc_src_dir)/gcc/BASE-VER`; \
	case $$base_ver in \
	12.* | 13.* | 14.* | 15.*) for f in $(gcc_vhdl_dir)/*.c; do mv $$f $${f}c; done; \
...
ghwdump$(EXEEXT): $(GHWDUMP_OBJS)
	$(CC) $(LDFLAGS) -o $@ $(GHWDUMP_OBJS) $(COVERAGE_FLAGS)
